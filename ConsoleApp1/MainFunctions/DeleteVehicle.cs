...
namespace CarDealership.MainFunctions
{
    internal class DeleteVehicle
    {
        ...
        public static void DeleteForPurchasedVehicle(string filePath, string vehicleType)
        {
            ...
            Console.Write("\nВведіть id транспорту, який хочете купити або -1, щоб повернутись до головного меню");
            Console.ForegroundColor= ConsoleColor.Red;
            Console.WriteLine("Застереження! Потім змінити обраний транспорт не можна!");
            Console.ResetColor();
            int idToDelete = Convert.ToInt32(Console.ReadLine());
            if(idToDelete == -1)
            {
            ...
            if (indexToDelete >= 0)
            {
                ...
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.WriteLine($"\nТранспорт з айді {idToDelete} успішно обраний.");
                Console.ResetColor();
            }
            else
            {
